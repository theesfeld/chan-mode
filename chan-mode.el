...
(defun chan--get-boards ()
  ...
  (or chan--boards-cache
      (setq chan--boards-cache
            (alist-get 'boards (chan--fetch-json (concat chan-base-url "boards.json"))))))
...
(defun chan--fetch-image (url &optional full-size)
  ...
  (with-temp-buffer
    (url-insert-file-contents url)
    (create-image (buffer-string) nil t :max-width (if full-size nil 200))))

;;; Catalog View
(defun chan-catalog (board)
  "Display catalog for BOARD."
  (interactive (list (completing-read "Board: "
      ...
      (let ((inhibit-read-only t))
        ...
        (dolist (post (alist-get 'posts thread))
          (let* ((no (alist-get 'no post))
            ...
            (shr-insert-document (shr-dom-from-html com))
            (insert "\n")
            (when thumbnail
              (let ((pos (point)))
...
(defun chan-thread-image-view ()
  ...
  (let ((buffer (get-buffer-create (format "*chan-images-%s-%d*"
                                           chan--current-board
    ...
    (with-current-buffer buffer
      (let ((inhibit-read-only t))
        ...
        (setq chan--thread-images (reverse chan--thread-images)) ;; Reverse for natural order
...
(defun chan--image-view-show-image (index)
  ...
  (let ((inhibit-read-only t))
    ...
    (let* ((img-data (nth index chan--thread-images))
      ...
      (insert (propertize (format "\nImage %d of %d" (1+ index) (length chan--thread-images))
                          'face 'italic)))
    (goto-char (point-min))
